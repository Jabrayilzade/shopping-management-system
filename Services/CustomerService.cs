...
namespace ShoppingDB.Services
{
    public class CustomerService : ICustomerService
    {
        ...
        public CustomerService(AppDataContext context, IMapper mapper, IDbConnection dbConnection)
        {
            this.mapper = mapper;
        ...
        {
            using (NpgsqlConnection connection = dbConnection.CreateDbConnection())
            {
                ...
                //var basket = connection.Query<Basket>("select ProductsId from baskets where baskets.customer_id = (select user_id from customers where customers.user_id = @current_user_id)", new { current_user_id = currentUserId });
                //await connection.ExecuteAsync("select array_append(baskets.ProductsId, @product_id) where baskets.customer_id = (select user_id from customers where customers.user_id = @current_user_id", new { product_id = product.Id, current_user_id = currentUserId });
